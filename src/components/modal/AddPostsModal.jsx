const AddPostModal = ({ _data, _submit, _onChange, _close, _categories }) => {
    return (
        <div className="fixed inset-0 flex justify-center items-center bg-primary-dark/70 dark:bg-primary/70 max-sm:overflow-scroll">
            <div className="bg-primary dark:bg-primary-dark border rounded-lg p-5 lg:w-11/12">
                <form className="grid lg:grid-cols-12 gap-4" onChange={_onChange} onSubmit={_submit}>
                    <label className="lg:col-span-3 lg:row-span-3 relative rounded-lg overflow-hidden group border border-black dark:border-white aspect-square">
                        ...
                        <input type="file" name="image" className="absolute inset-0 rounded-lg opacity-0 cursor-pointer" accept="image/png, image/jpeg, image/webp" required />
                    </label>
                    <label className="lg:col-span-9">
                        Title
                        <input type="text" name="title" defaultValue={_data?.title} required minLength={1} />
                    </label>
                    <label className="lg:col-span-3">
                        Category
                        <select className="w-full" name="categoryId" defaultValue={_data?.categoryId} required>
                    ...
                    <label className="lg:col-span-3">
                        Date
                        <input type="date" name="date" defaultValue={_data?.date} required />
                    </label>
                    <label className="lg:col-span-9">
                        Content
                        <textarea name="content" defaultValue={_data?.content} rows={8} required minLength={1}></textarea>
                    </label>
